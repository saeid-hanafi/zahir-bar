	...
	// print_r($atts);
		...
		if ( $q->have_posts() ) {
			...
			while ( $q->have_posts() ) {
				...
				$classes = 'class="post '.join( " " , get_post_class( $classes , get_the_id() ) ).'"';
					...
					if ( $gallery_images !== false ) {
						...
						if ( is_array( $gallery_ids ) ) {
							...
							foreach ( $gallery_ids as $image ) {
								...
								$gallery_markup .='<a href="'.esc_attr( get_permalink() ).'">';
								$gallery_markup .= wp_get_attachment_image($image , $img_size, false, ['alt' => get_the_title( $image )] );
								$gallery_markup .='</a>';

								if($no_link_overlay != 'yes'){
									$gallery_markup .='		<a class="item-link-overlay" href="'.esc_attr( get_permalink() ).'" '.$overlay.'></a>';
								}else{
									$gallery_markup .='		<div class="item-link-overlay" '.$overlay.'></div>';
								}
								$gallery_markup .='		<div class="wbc-extra-links">';

								if($hide_popup_link != 'yes'){
									$gallery_markup .='			<a data-fancybox="fancy-lightbox[gallery-'.$id.'-'.$wbc_blog_count.']"  title="'.esc_attr( get_the_title( $image ) ).'" href="'.esc_attr( $large_path[0] ).'" data-thumb="'.esc_attr( $path[0] ).'" class="wbc-photo-up"><i class="fa fa-search"></i></a>';
								}
				...
				default:
					...
					if ( has_post_thumbnail() ) {
						...
						if($no_link_overlay != 'yes'){
							...
							$html .='		<div class="item-link-overlay" '.$overlay.'></div>';
						}

						$html .='		<div class="wbc-extra-links">';
						
						if($hide_popup_link != 'yes'){
							$html .='			<a data-fancybox title="'.get_the_title( get_post_thumbnail_id( $id ) ).'" href="'.esc_attr( $large_image_url[0] ).'" class="wbc-photo-up"><i class="fa fa-search"></i></a>';
						}
				...
				if ( get_post_format() != 'quote' && get_post_format() != 'link' ) {
					...
					$html .='<span class="date"><i class="far fa-calendar-alt"></i> '.get_the_date( get_option( 'date_format' ) ).'</span>';
					$html .='<span class="user"><i class="fas fa-user"></i> '.esc_html__( 'By', 'ninezeroseven' ).' <a href="'.esc_attr( get_author_posts_url( get_the_author_meta( 'ID' ) ) ).'">'.get_the_author_meta( 'display_name' ).'</a></span>';


					if ( get_post_type() == 'post' ) {
						$html .='<span class="post-in"><i class="fas fa-pencil-alt"></i> '.esc_html__( 'In', 'ninezeroseven' ).' ';
					...
					$html .='<span class="comments"><i class="fas fa-comments"></i> ';
				...
				if ( get_post_format() == 'quote' || get_post_format() == 'link' ) {
					...
					if( $hide_dots == 'yes' ){
						$html .= str_replace('...','', wbc_get_excerpt( $excerpt_length ));
					}else{
