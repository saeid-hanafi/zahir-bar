...
if(defined('VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG')){
	function wbc907_vc_shortcode_filter($classes = '', $base ='', $atts='' ){
		if(!isset($base) || empty($base)) return $classes;

		if( $base == 'vc_single_image' ){
			if(is_array($atts) && isset($atts['display_prop']) && $atts['display_prop'] == 'inline'){
				return $classes.' wbc-display-inline';
			}
		}
	...
	add_filter( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'wbc907_vc_shortcode_filter', 10, 3);
}

/************************************************************************
* ULT Addon
*************************************************************************/
if(!function_exists('wbc907_add_ult_icon_manager')){
	function wbc907_add_ult_icon_manager( ) {
		if(!class_exists('Ultimate_Icon_Manager_Param')){
			return;
		}
		
		$change_sc = array('wbc_icon',
							'wbc_icon_box',
							'wbc_list_item'
							);
		
		foreach ($change_sc as $shortcode) {

	  		$mapshortcode = WPBMap::getShortCode($shortcode);
	  		
	  		if(isset($mapshortcode['params']) && is_array($mapshortcode['params'])){
				$new_map = array();

		  		foreach ($mapshortcode['params'] as $key => $value) {

		  			if( $value['param_name'] == 'icon_pack' ){
		  			
			  			$value['value']['ULT Icon Manager'] = 'ult_icons';
			  			...
			  			$new_map[] = array(
							"type"        => "icon_manager",
							"class"       => "",
							"heading"     => __("Select Icon ","ninezeroseven"),
							"param_name"  => "icon",
							"value"       => "",
							"admin_label" => true,
							"description" => __("Click and select icon of your choice. If you can't find the one that suits for your purpose","ninezeroseven").", ".__("you can","ninezeroseven")." <a href='admin.php?page=bsf-font-icon-manager' target='_blank' rel='noopener'>".__("add new here","ninezeroseven")."</a>.",
							'dependency'  => array(
											'element' => 'icon_pack',
											'value'   => 'ult_icons',
											),
		  		...
		  		$paramse = WPBMap::modify( $shortcode ,'params', $new_map);
	  		}
	 	}

	}

	add_action( 'vc_after_init', 'wbc907_add_ult_icon_manager',200);
}
...
if(!function_exists('wbc907_add_primaray_color_vc')){
	function wbc907_add_primaray_color_vc() {

	  $param = WPBMap::getParam( 'vc_tta_accordion', 'color' );

	  $param['value'][__( 'Theme Primary Color', 'ninezeroseven' )] = 'wbc-theme-primary-color';
	...
	add_action( 'vc_after_init', 'wbc907_add_primaray_color_vc' );
}
...
if ( !function_exists( 'wbc_row_menu_bar_output' ) ) {
	function wbc_row_menu_bar_output( $output , $shortcode_class ) {
		...
		if ( $className == 'WPBakeryShortCode_VC_Row' || $className == 'WPBakeryShortCode_Vc_Row' && !is_admin() ) {
...
if ( !function_exists( 'wbc907buildStyle' ) ) {
	function wbc907buildStyle( $bg_image = '', $bg_color = '', $bg_image_repeat = '', $font_color = '', $padding = '', $margin = '', $image_position ='' , $image_attach ='', $border_color = '' ) {
		...
		}elseif(!empty( $bg_image ) && 1 === preg_match('/[\w\-]+\.(jpg|png|gif|jpeg)/i', $bg_image)){
			$has_image = true;
		...
		if ( ! empty( $image_attach ) && $has_image ) {
			$style .= 'background-attachment:'.$image_attach.';';
		}
...
if ( !function_exists( 'wbc907_map_to_vc' ) ) {
	function wbc907_map_to_vc() {
		...
		vc_add_param("vc_row_inner",array(
			'type'        => 'checkbox',
			'heading'     => esc_html__( 'Match By Row?', 'ninezeroseven' ),
			'param_name'  => 'match_row',
			'description' => esc_html__( 'If selected, height will match by row', 'ninezeroseven' ),
			'value'       => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => 'yes' ),
			"dependency" => array('element' => "match_height", 'value' => array('yes')),
		));
		...
		vc_add_param("vc_column", array(
			"type" => "dropdown",
			"class" => "",
			"heading" => esc_html__('Image Source', 'ninezeroseven'),
			"description" => esc_html__('Load image from upload, or url.', 'ninezeroseven'),
			"param_name" => "source",
			"value" => array(
				esc_html__("Media Library",'ninezeroseven') => '',
				esc_html__("External Link",'ninezeroseven') => "external_link",
			),
			'std' => '',
		...
		vc_add_param("vc_column", array(
			...
			"dependency"  => array('element' => "source", 'value_not_equal_to' => 'external_link'),
			'group' => esc_html__( '907 Options', 'ninezeroseven' )
		));

		vc_add_param("vc_column",array(
			'type'        => 'textfield',
			'heading'     => esc_html__( 'Full Image URL', 'ninezeroseven' ),
			'param_name'  => 'custom_src',
			"dependency"  => array('element' => "source", 'value' => array('external_link')),
			'description' => esc_html__( 'Must input full valid image URL', 'ninezeroseven' ),
			'group'       => esc_html__( '907 Options', 'ninezeroseven' )
		));
		vc_add_param("vc_column", array(
		    'type' => 'dropdown',
		...
		*************************************************************************/
		...
		vc_add_param("vc_row", array(
			...
			"heading" => esc_html__('Bg Image', 'ninezeroseven'),
			"param_name" => "parallax_img",
			"dependency"  => array('element' => "source", 'value_not_equal_to' => array('external_link')),
			'group' => esc_html__( '907 Options', 'ninezeroseven' )
		));
		...
		*************************************************************************/
		...
		vc_add_param("vc_row",array(
			...
			'description'    => esc_html__( 'Best to have videos muted, as some browser like Safari, Iphone and soon Chrome don\'t support autoplay while unmuted.', 'ninezeroseven' ),
		...
		vc_add_param("vc_row",array(
			'type'           => 'checkbox',
			'heading'        => esc_html__( 'Lazy Load?', 'ninezeroseven' ),
			'param_name'     => 'video_lazy_load',
			'description' => esc_html__( 'This will delay the loading of the video till scrolled down close to the video area.', 'ninezeroseven' ),
			"dependency"     => array('element' => "bg_select", 'value' => array('bg_video','bg_youtube_video')),
			'value'          => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => true ),
			'group'          => esc_html__( '907 Options', 'ninezeroseven' )
		));
		vc_add_param("vc_row",array(
			'type'        => 'textfield',
			'heading'     => esc_html__( 'Load Position Delay', 'ninezeroseven' ),
			'param_name'  => 'video_lazy_offset',
			'description' => esc_html__( 'Distance from video area to begin loading it, default 500', 'ninezeroseven' ),
			"dependency"     => array('element' => "video_lazy_load", 'value' => array('1')),
			'group' => esc_html__( '907 Options', 'ninezeroseven' )
		));
		vc_add_param("vc_row",array(
			'type'           => 'checkbox',
			'heading'        => esc_html__( 'Play When InView?', 'ninezeroseven' ),
			'param_name'     => 'video_play_inview',
			'description'    => esc_html__( 'Will play the video when inview, pause when not inview.', 'ninezeroseven' ),
			"dependency"     => array('element' => "bg_select", 'value' => array('bg_video','bg_youtube_video')),
			'value'          => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => true ),
			'group'          => esc_html__( '907 Options', 'ninezeroseven' )
		));
		...
		vc_add_param("vc_row",array(
			'type'        => 'textfield',
			'heading'     => esc_html__( 'Start Playing Position', 'ninezeroseven' ),
			'param_name'  => 'video_offset_play',
			'description' => esc_html__( 'Amount of video shown before starting to play. Must enter percentage value ie 50%, will start play when 50% is visible, if used without percentage sign video will start that amount before the video is shown. ', 'ninezeroseven' ),
			"dependency"     => array('element' => "video_play_inview", 'value' => array('1')),
			'group' => esc_html__( '907 Options', 'ninezeroseven' )
		));
		vc_add_param("vc_row",array(
			'type'        => 'textfield',
			'heading'     => esc_html__( 'Stop Playing Position', 'ninezeroseven' ),
			'param_name'  => 'video_offset_bottom',
			'description' => esc_html__( 'Distance scrolled out of view to stop playing video. Enter percentage value, 40% would pause video once 3/4 is out of view when scrolling down. Leave blank and video will pause once completly out for view', 'ninezeroseven' ),
			"dependency"     => array('element' => "video_play_inview", 'value' => array('1')),
			'group' => esc_html__( '907 Options', 'ninezeroseven' )
		));
		...
		vc_add_param("vc_row",array(
			'type'           => 'checkbox',
			'heading'        => esc_html__( 'Play On Mobile?', 'ninezeroseven' ),
			'param_name'     => 'video_play_mobile',
			'description'    => esc_html__( 'Enable this will attempt to play the video backgrounds on mobile devices if supported. Consider user data when enabling this option.', 'ninezeroseven' ),
			"dependency"     => array('element' => "bg_select", 'value' => array('bg_video','bg_youtube_video')),
			'value'          => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => true ),
			'group'          => esc_html__( '907 Options', 'ninezeroseven' )
		));
		...
		vc_add_param("vc_row", array(
		    'type' => 'dropdown',
		    'heading' => esc_html__( 'Video Overprint', 'ninezeroseven' ),
		    'param_name' => 'video_expand', //or “small”, “medium”, “large”, “hd720”, “hd1080”, “highres”
		    'value' => array(
							 '0' => '0',
							 '0.1' => '0.1',
							 '0.2' => '0.2',
		    ...
		    'description' => esc_html__( 'Expands video over section so there are no gaps, 0.3+ works best for 21/9 resolution videos lower for 16/9.', 'ninezeroseven' ),
		    'std' => '0.3',
		    'dependency' => array(
		    	'element' => "bg_select", 'value' => array('bg_youtube_video'),
				),
		...
		vc_add_param("vc_single_image", array(
		    'type' => 'dropdown',
		    'heading' => esc_html__( 'Display Property', 'ninezeroseven' ),
		    'param_name' => 'display_prop',
		    'value' => array(
							esc_html__( 'Default', 'ninezeroseven' )      => '',
							esc_html__( 'Inline Block', 'ninezeroseven' ) => 'inline',
						),
		    'description' => esc_html__( 'Set to Inline Block when trying to align images next to each other.', 'ninezeroseven' )
		));
		...
		vc_map(array(
			...
			'params'=>array(
				...
				array(
					'type' => 'checkbox',
					'heading' => esc_html__( 'Hide Post Title?', 'ninezeroseven' ),
				...
				array(
					'type' => 'checkbox',
					'heading' => esc_html__( 'Hide PopUp Link Icon?', 'ninezeroseven' ),
					'param_name' => 'hide_popup_link',
					// 'description' => esc_html__( 'If selected, this will set min-height to browser height.', 'ninezeroseven' ),
					'value' => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => 'yes' ),
				),
				array(
					'type' => 'checkbox',
					'heading' => esc_html__( 'Hide Page Link Icon?', 'ninezeroseven' ),
					'param_name' => 'hide_page_link',
					// 'description' => esc_html__( 'If selected, this will set min-height to browser height.', 'ninezeroseven' ),
					'value' => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => 'yes' ),
				),
				array(
					'type' => 'checkbox',
					'heading' => esc_html__( 'UnLink Overlay?', 'ninezeroseven' ),
					'param_name' => 'no_link_overlay',
					'description' => esc_html__( 'Makes overlay become not clickable to single post page.', 'ninezeroseven' ),
					'value' => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => 'yes' ),
				),
				array(
					'type' => 'checkbox',
					'heading' => esc_html__( 'Group Images in Lightbox?', 'ninezeroseven' ),
					'param_name' => 'group_lightbox',
					'description' => esc_html__( 'All portfolio items displayed will be grouped in the lightbox.', 'ninezeroseven' ),
					'dependency' => array( 'element' => 'hide_popup_link', 'value_not_equal_to' => 'yes'),
					'value' => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => 'yes' ),
				),
				array(
					'type' => 'checkbox',
				...
				array(
					'type' => 'checkbox',
					'heading' => esc_html__( 'Paginate By Filter Buttons?', 'ninezeroseven' ),
					'param_name' => 'paginate_by_filter',
					'admin_label' => true,
					'description' => esc_html__( 'If you have large portfolio, you can select to paginate it based on filter buttons.', 'ninezeroseven' ),
				...
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Excerpt Length', 'ninezeroseven' ),
				...
				array(
					'type' => 'checkbox',
					'heading' => esc_html__( 'Hide Dots (...)?', 'ninezeroseven' ),
					'param_name' => 'hide_dots',
					'description' => esc_html__( 'Hides the 3 dots on excerpt.', 'ninezeroseven' ),
					'value' => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => 'yes' ),
					"dependency" => array('element' => "portfolio_display", 'value' => array('yes')),
				),
				array(
				...
				array(
					'type'        => 'checkbox',
					'heading'     => esc_html__( 'Mute Videos?', 'ninezeroseven' ),
					'param_name'  => 'mute_videos',
					'description' => esc_html__( 'Will mute the videos.', 'ninezeroseven' ),
					'value'       => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => 'yes' ),
					"dependency" => array('element' => "mouse_over_play", 'value' => array('yes')),
				),
				array(
					'type' => 'colorpicker',
					'heading' => esc_html__( 'Overlay Color', 'ninezeroseven' ),
				...
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Overlay Title Font Size', 'ninezeroseven' ),
					'param_name'  => 'title_size',
					'description' => esc_html__( 'Font size for heading (ie 18px)', 'ninezeroseven' ),
					'group'       => esc_html__( 'Styling', 'ninezeroseven' )
				),
				array(
					'type' => 'colorpicker',
					'heading' => esc_html__( 'Overlay Title Color', 'ninezeroseven' ),
					'param_name' => 'title_color',
					'description' => esc_html__( 'Heading Font Color', 'ninezeroseven' ),
					'group'       => esc_html__( 'Styling', 'ninezeroseven' )

				),
				array(
					'type' => 'colorpicker',
					'heading' => esc_html__( 'Overlay Icons Color', 'ninezeroseven' ),
					'param_name' => 'icon_color',
					'description' => esc_html__( 'Changes color of icons', 'ninezeroseven' ),
					'group'       => esc_html__( 'Styling', 'ninezeroseven' )
				...
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Below 400', 'ninezeroseven' ),
					'param_name'  => 'cols_xxs',
					'description' => esc_html__( 'Columns to display in area below 400 in width, enter only a number i.e 4', 'ninezeroseven' ),
					'group'       => esc_html__( 'Column Settings', 'ninezeroseven' )
				),
			),
		));
		...
		vc_map(array(
			...
			'params'=>array(
				...
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Scroll Speed', 'ninezeroseven' ),
					'param_name'  => 'item_speed',
					'description' => esc_html__( 'This controls scroll speed. Enter numbers only, default 1000', 'ninezeroseven' ),
					'group'       => esc_html__( 'Settings', 'ninezeroseven' )
				),
				array(
					'type'        => 'textfield',
		...
		vc_map(array(
			...
			'params'=>array(
				...
				array(
					...
					'heading'     => esc_html__( 'Below 400', 'ninezeroseven' ),
					'param_name'  => 'cols_xxs',
					'description' => esc_html__( 'Columns to display in area below 400 in width when using "Masonry" layout option., enter only a number i.e 4', 'ninezeroseven' ),
					'group'       => esc_html__( 'Column Settings', 'ninezeroseven' )
				),
			),
		...
		vc_map( array(
		    ...
		    "params" => array(
		        ...
		        array(
					...
					'param_name'  => 'ref_name',
					'description' => esc_html__( 'Only for your reference when using page builder', 'ninezeroseven' ),
					'admin_label' => true,
				),
				array(
					'type'        => 'checkbox',
					'heading'     => esc_html__( 'Show Name?', 'ninezeroseven' ),
					'param_name'  => 'show_name',
					'dependency'  => array( 'element' => 'ref_name', 'not_empty' => true),
					'description' => esc_html__( 'Will show name field on hover', 'ninezeroseven' ),
					'value'       => array( esc_html__( 'Yes, please', 'ninezeroseven' ) => 'yes' )
				),
		        array(
					"type" => "dropdown",
		...
		vc_map( array(
		    ...
		    "params" => array(
		            ...
		            array(
		                ...
		                'heading'     => esc_html__( 'Link', 'ninezeroseven' ),
		                'param_name'  => 'link',
		                'description' => esc_html__( 'Wraps title content in a link tag.', 'ninezeroseven' ),
		            ),
		            array(
		                'type' => 'dropdown',
		...
		vc_map( array(
		    ...
		    "params" => array(
		            ...
		            array(
						...
						'heading'     => esc_html__( 'Link', 'ninezeroseven' ),
						'param_name'  => 'link',
						'description' => esc_html__( 'If you\'d like the image linked', 'ninezeroseven' ),
					),
		            array(
		                'type' => 'textarea',
		                'holder' => 'div',
		                'heading' => esc_html__( 'Member Text', 'ninezeroseven' ),
		                'description' => esc_html__( 'HTML may cause this field to not show.', 'ninezeroseven' ),
		            ...
		            array(
		                ...
		                'param_name' => 'font_color',
		                'description' => esc_html__( 'Changes description/bio text color.', 'ninezeroseven' ),
		...
		vc_map( array(
		    ...
		    "params" => array(
		            ...
		            array(
		                'type' => 'colorpicker',
		                'heading' => esc_html__( 'Box Shadow Color', 'ninezeroseven' ),
		                'param_name' => 'box_shadow_color',
		                'description' => esc_html__( 'Add a box shadow color', 'ninezeroseven' ),

		            ),
		            array(
		                'type'        => 'textfield',
		                'heading'     => esc_html__( 'Box Shadow X Offset', 'ninezeroseven' ),
		                'param_name'  => 'box_shadow_x',
		                'dependency'  => array( 'element' => 'box_shadow_color', 'not_empty' => true),
		                'description' => esc_html__( 'Offset x for box shadow', 'ninezeroseven' ),
		                // 'group'       => esc_html__( 'Styling', 'ninezeroseven' )
		            ),
		            array(
		                'type'        => 'textfield',
		                'heading'     => esc_html__( 'Box Shadow Y Offset', 'ninezeroseven' ),
		                'param_name'  => 'box_shadow_y',
		                'dependency'  => array( 'element' => 'box_shadow_color', 'not_empty' => true),
		                'description' => esc_html__( 'Offset y for box shadow', 'ninezeroseven' ),
		                // 'group'       => esc_html__( 'Styling', 'ninezeroseven' )
		            ),
		            array(
		                'type'        => 'textfield',
		                'heading'     => esc_html__( 'Box Shadow Blur', 'ninezeroseven' ),
		                'param_name'  => 'box_shadow_blur',
		                'dependency'  => array( 'element' => 'box_shadow_color', 'not_empty' => true),
		                'description' => esc_html__( 'Amount of box shadow blur', 'ninezeroseven' ),
		                // 'group'       => esc_html__( 'Styling', 'ninezeroseven' )
		            ),
		            array(
		                'type'        => 'textfield',
		                'heading'     => esc_html__( 'Box Shadow Spread', 'ninezeroseven' ),
		                'param_name'  => 'box_shadow_spread',
		                'dependency'  => array( 'element' => 'box_shadow_color', 'not_empty' => true),
		                'description' => esc_html__( 'Amount of box shadow spread', 'ninezeroseven' ),
		                // 'group'       => esc_html__( 'Styling', 'ninezeroseven' )
		            ),
		...
		vc_map( array(
			...
			"params"                  => array(
		            array(
						...
						'description' => esc_html__( 'Text to display on button.', 'ninezeroseven' ),
						'admin_label' => true,
		...
		vc_map( array(
		    ...
		    "params" => array(
				...
				array(
	                ...
	                'heading'     => esc_html__( 'Title Color', 'ninezeroseven' ),
	                'param_name'  => 'title_color',
	                'description' => esc_html__( 'Color option for the plan name at the top.', 'ninezeroseven' ),
	                'group'       => esc_html__( 'Color Options', 'ninezeroseven' )

	            ),
	            array(
	                'type'        => 'colorpicker',
	                'heading'     => esc_html__( 'Title Background Color', 'ninezeroseven' ),
	                'param_name'  => 'title_bg_color',
	                'description' => esc_html__( 'Color option for the background plan name at the top.', 'ninezeroseven' ),
	                'group'       => esc_html__( 'Color Options', 'ninezeroseven' )

	            ),
	            array(
	                'type'        => 'colorpicker',
	                'heading'     => esc_html__( 'Price Color', 'ninezeroseven' ),
	                'param_name'  => 'price_color',
	                'description' => esc_html__( 'Color option for the price/cost', 'ninezeroseven' ),
	                'group'       => esc_html__( 'Color Options', 'ninezeroseven' )

	            ),
	            array(
	                'type'        => 'colorpicker',
	                'heading'     => esc_html__( 'Price Background Color', 'ninezeroseven' ),
	                'param_name'  => 'price_bg_color',
	                'description' => esc_html__( 'Background color option for the price/cost', 'ninezeroseven' ),
	                'group'       => esc_html__( 'Color Options', 'ninezeroseven' )
	            ...
	            array(
	                ...
	                'heading'     => esc_html__( 'Font Color', 'ninezeroseven' ),
	                'param_name'  => 'text_color',
	                'description' => esc_html__( 'Color option for all fonts below the plan name.', 'ninezeroseven' ),
	                'group'       => esc_html__( 'Color Options', 'ninezeroseven' )

	            ),
	            array(
	                'type'        => 'colorpicker',
	                'heading'     => esc_html__( 'Plan Background Color', 'ninezeroseven' ),
	                'param_name'  => 'plan_bg_color',
	                'description' => esc_html__( 'Background color option for plan.', 'ninezeroseven' ),
	                'group'       => esc_html__( 'Color Options', 'ninezeroseven' )

	            ),
				array(
	                'type'        => 'colorpicker',
	                'heading'     => esc_html__( 'Button Color', 'ninezeroseven' ),
	                'param_name'  => 'button_color',
	                'description' => esc_html__( 'Change button color.', 'ninezeroseven' ),
	                'group'       => esc_html__( 'Color Options', 'ninezeroseven' )
