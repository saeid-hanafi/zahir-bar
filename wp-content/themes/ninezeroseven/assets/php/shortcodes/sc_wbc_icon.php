...
$atts = extract( shortcode_atts(
		...
		if( $has_custom == true ){
			...
			$content_add .= '<img src="'. esc_attr( $custom_src ).'" alt="'.esc_html__("icon image",'ninezeroseven').'">';
		}else if( $icon_image && $display_type == 'img'){
			$iconClasses .= ' icon-img';
			$content_add .= wp_get_attachment_image($icon_img , 'full', false, ['alt' => get_the_title( $icon_img )] );
		}else{
